...
const adminUser = {
    secretWord: "FakeStore", email: "admin@fake.com", password: "12345678"
}
...
async function getAdminUser(email, password) {
    return (await axios.post(urlAdmin, {
        email, password, secretword: `${adminUser.secretWord}`
    })).data.token
}
...
async function setupBrandEngine(token) {
    for (let i of brandsSetUp) {
        try {
            await axios.post(urlBrand, {name: i}, {
                headers: {
    ...
    for (let i of enginesSetUp) {
        try {
            await axios.post(urlEngine, {name: i}, {
                headers: {
                    'Content-Type': 'multipart/form-data', Authorization: `Bearer ${token}`
                }
            }).catch(e => {
    ...
    for (let car of carsInfoSetUp) {
        try {
            await axios.post(urlCarInfo, {
                carId: car.carId, horsepower: car.horsepower, topspeed: car.topspeed, engine: car.engine,
            }, {
                headers: {
                    'Content-Type': 'multipart/form-data', Authorization: `Bearer ${token}`
                }
...
(async () => {
    ...
    setupBrandEngine(token).then(() => {
        setupCars(token).then(() => {
            setupImagesInfo(token).then(() => console.log("All items had successfully added!\nIf you get status code 404, that means any of items have already been added or something went wrong.")).catch(e => {
                e.message
            })
